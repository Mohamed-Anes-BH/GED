...
import { Archive, Search, Plus, Edit2, Trash2, X, Loader2 } from 'lucide-react';
import { useOrganizationCrud } from '../hooks/useOrganization';
import { sitesService } from '../services/organization';
...
export function ArchivesPhysiquesPage() {
  const {
    ...
    searchQuery,
    setSearchQuery
  } = useOrganizationCrud(sitesService);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce site d'archivage ?")) {
    ...
    <div className="flex flex-col gap-6 font-poppins pb-10 text-gray-800 dark:text-[var(--dash-text)] h-[calc(100vh-140px)]">
      <div className="flex items-start gap-4">
        <div className="w-14 h-14 bg-gray-100 border border-gray-200 dark:border-[var(--dash-border)] rounded-2xl flex items-center justify-center shadow-sm shrink-0">
          <Archive size={26} className="text-gray-605 dark:text-[var(--dash-text-muted)]" />
        </div>
        <div>
          <h2 className="text-3xl font-bold font-oswald text-gray-900 dark:text-[#FFFFFF]">Archives physiques (Sites)</h2>
        ...
        <div className="overflow-auto custom-scrollbar flex-1">
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} className="py-4 text-center text-gray-500">Chargement...</td>
                </tr>
              ) : sites.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-gray-500">Aucun site trouvé.</td>
                </tr>
              ) : (
                sites.map(s => (
                  <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-[var(--dash-border)] dark:bg-[var(--dash-bg)]/50">
                    <td className="py-3 px-5 font-bold text-gray-800 dark:text-[var(--dash-text)]">{s.name}</td>
                    <td className="py-3 px-5">{s.address || '—'}</td>
                    <td className="py-3 px-5">{s.description || '—'}</td>
                    <td className="py-3 px-5 text-right flex justify-end gap-2">
                      <button onClick={() => handleOpenModal(s)} className="p-1 hover:text-orange-500 transition-colors">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => handleDelete(s.id)} className="p-1 hover:text-red-500 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </td>
                  </tr>
          ...
          <div className="bg-white dark:bg-[var(--dash-card-bg)] rounded-2xl shadow-lg w-full max-w-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-[var(--dash-border)]">
              <h3 className="font-bold text-gray-900 dark:text-[#FFFFFF]">
                {editingItem ? 'Modifier le site' : 'Nouveau site d\'archivage'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5 align-start text-left">
                <label className="text-[11px] font-bold text-gray-600 dark:text-[var(--dash-text-muted)]">Nom du site</label>
                <input
                  required
                  value={formData.name}
              ...
              <div className="flex flex-col gap-1.5 align-start text-left">
                <label className="text-[11px] font-bold text-gray-650 dark:text-[var(--dash-text-muted)]">Description</label>
                <textarea
                  value={formData.description}
                  onChange={e => setFormData({ ...formData, description: e.target.value })}
                  className="px-3 py-2 border border-gray-200 dark:border-[var(--dash-border)] rounded-lg text-sm bg-transparent outline-none focus:border-orange-500 h-20 resize-none"
...
function CheckCircle(props: any) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2500/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
