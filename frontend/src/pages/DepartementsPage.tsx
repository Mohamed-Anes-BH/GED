...
import { departementsService, directionsService } from '../services/organization';

export function DepartementsPage() {
  const { items: departements, loading, create, update, remove, searchQuery, setSearchQuery } = useOrganizationCrud(departementsService);
  const [directions, setDirections] = useState<any[]>([]);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<any>(null);
  
  const [formData, setFormData] = useState({ name: '', code: '', direction: '', status: 'actif' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    directionsService.getAll({ page_size: 1000 }).then(data => setDirections(data.results || data)).catch(console.error);
  }, []);

  const handleOpenModal = (item?: any) => {
    if (item) {
      setEditingItem(item);
      setFormData({ name: item.name, code: item.code || '', direction: item.direction || (item.direction_details?.id) || '', status: item.status || 'actif' });
    } else {
      setEditingItem(null);
      setFormData({ name: '', code: '', direction: '', status: 'actif' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.direction) return alert('Le nom et la direction sont obligatoires.');
  ...
  return (
    <div className="flex flex-col gap-6 font-poppins pb-10 text-gray-800 dark:text-[var(--dash-text)] h-[calc(100vh-140px)]">
      <div className="flex items-start gap-4">
        <div className="w-14 h-14 bg-blue-50 border border-blue-100 rounded-2xl flex items-center justify-center shadow-sm shrink-0">
          <Building2 size={26} className="text-blue-500" />
        </div>
        <div>
          <h2 className="text-3xl font-bold font-oswald text-gray-900 dark:text-[#FFFFFF]">Départements</h2>
          <div className="flex items-center gap-1 text-[11px] font-medium text-gray-400 mt-0.5">
            <span>Accueil</span> <span>›</span> <span className="text-orange-500 font-semibold">Départements</span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            <thead className="sticky top-0 bg-gray-50 dark:bg-[var(--dash-bg)] shadow-sm">
                <tr className="bg-gray-50 dark:bg-[var(--dash-bg)] border-b border-gray-100 dark:border-[var(--dash-border)] text-gray-500 font-bold">
                    ...
                    <th className="py-3 px-5">Direction Parent</th>
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} className="py-4 text-center text-gray-500">Chargement...</td></tr> : 
                departements.length === 0 ? <tr><td colSpan={6} className="py-8 text-center text-gray-500">Aucun département trouvé.</td></tr> :
                departements.map(d=><tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-[var(--dash-border)] dark:bg-[var(--dash-bg)]/50">
                    <td className="py-3 px-5 font-semibold text-blue-600">{d.code || '—'}</td>
                    <td className="py-3 px-5 font-bold text-gray-800 dark:text-[var(--dash-text)]">{d.name}</td>
                    <td className="py-3 px-5 text-gray-600 dark:text-[var(--dash-text-muted)]">{d.direction_details?.name || '—'}</td>
                    <td className="py-3 px-5">{d.manager_details ? `${d.manager_details.first_name || ''} ${d.manager_details.last_name || ''}`.trim() : '—'}</td>
                    <td className="py-3 px-5 text-center">
                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold ${d.status === 'actif' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{d.status === 'actif' ? 'Actif' : 'Inactif'}</span>
                    </td>
                    <td className="py-3 px-5 text-right flex justify-end gap-2">
                        <button onClick={() => handleOpenModal(d)} className="p-1 hover:text-orange-500 transition-colors"><Edit2 size={14}/></button>
                        <button onClick={() => handleDelete(d.id)} className="p-1 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                    </td>
                </tr>)}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-[var(--dash-card-bg)] rounded-2xl shadow-lg w-full max-w-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-[var(--dash-border)]">
               <h3 className="font-bold text-gray-900 dark:text-[#FFFFFF]">{editingItem ? 'Modifier le département' : 'Nouveau département'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
               ...
               <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-gray-600 dark:text-[var(--dash-text-muted)]">Nom du département</label>
                  <input required value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} type="text" className="px-3 py-2 border border-gray-200 dark:border-[var(--dash-border)] rounded-lg text-sm bg-transparent outline-none focus:border-orange-500" placeholder="Ex: Ressources Humaines" />
               </div>
               <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-gray-600 dark:text-[var(--dash-text-muted)]">Direction Parente</label>
                  <select required value={formData.direction} onChange={e=>setFormData({...formData, direction: e.target.value})} className="px-3 py-2 border border-gray-200 dark:border-[var(--dash-border)] rounded-lg text-sm bg-transparent outline-none focus:border-orange-500">
                    <option value="" disabled className="dark:bg-gray-800">Sélectionner une direction</option>
                    {directions.map(dir => (
                        <option key={dir.id} value={dir.id} className="dark:bg-gray-800">{dir.name} ({dir.code})</option>
                    ))}
                  </select>
               </div>
               <div className="flex flex-col gap-1.5">
