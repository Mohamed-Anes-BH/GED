...
export function HistoriquePage() {
  const actions = [
    { type: 'Création', icon: Plus, color: 'text-green-500', bg: 'bg-green-50', border: 'border-green-100', user: 'Sophie Martin', date: '24/05/2026', time: '14:32:15', details: 'Document "Rapport_Annuel_2024.docx" créé' },
    { type: 'Modification', icon: Edit2, color: 'text-orange-500', bg: 'bg-orange-50', border: 'border-orange-100', user: 'Julien Moreau', date: '24/05/2026', time: '11:15:08', details: 'Document "Facture_2026_001.pdf" modifié' },
    { type: 'Consultation', icon: Eye, color: 'text-blue-500', bg: 'bg-blue-50', border: 'border-blue-100', user: 'Claire Dubois', date: '23/05/2026', time: '16:45:37', details: 'Consultation du document "Dossier_Contrats_2024"' },
    { type: 'Téléchargement', icon: Download, color: 'text-purple-500', bg: 'bg-purple-50', border: 'border-purple-100', user: 'Marc Lefevre', date: '23/05/2026', time: '10:22:41', details: 'Téléchargement du document "Budget_Previsionnel.xlsx"' },
    { type: 'Diffusion', icon: Share, color: 'text-red-500', bg: 'bg-red-50', border: 'border-red-100', user: 'Nathalie Bernard', date: '22/05/2026', time: '09:12:30', details: 'Diffusion du document "Courrier_Client_ACME.eml" à 5 utilisateurs' },
    { type: 'Archivage', icon: Archive, color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'border-yellow-200', user: 'Admin AgrOdiv', date: '21/05/2026', time: '13:40:22', details: 'Archivage du document "Archives_2019"' },
    { type: 'Restauration', icon: RefreshCw, color: 'text-teal-500', bg: 'bg-teal-50', border: 'border-teal-100', user: 'Sophie Martin', date: '21/05/2026', time: '11:08:54', details: 'Restauration du document "Plan_Projet_Agrodiv.pdf"' },
    { type: 'Suppression', icon: Trash2, color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-100', user: 'Julien Moreau', date: '20/05/2026', time: '15:56:10', details: 'Suppression du document "Note_Interne_0526.docx"' },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 font-poppins pb-10 text-gray-800">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <h2 className="text-3xl font-bold font-oswald text-gray-900 dark:text-gray-100">Historique</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex flex-wrap gap-4 items-end">
        <div className="flex flex-col gap-1 flex-1 min-w-[150px]">
          ...
          <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 bg-white">
            ...
            <select className="flex-1 outline-none text-xs font-medium text-gray-700 bg-transparent"><option>Toutes les actions</option></select>
          </div>
        </div>
        <div className="flex flex-col gap-1 flex-1 min-w-[150px]">
        ...
        <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
          ...
          <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 bg-white">
            ...
            <div className="flex-1 text-xs font-medium text-gray-700 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button className="flex items-center gap-2 px-5 py-2.5 border border-gray-200 bg-white text-gray-700 font-semibold text-[13px] rounded-xl shadow-sm hover:bg-gray-50"><FileText size={15}/> Export CSV</button>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            ...
            <tbody>
              {actions.map((act, i) => (
                <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/80 group">
                  <td className="py-3 px-6">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center border ${act.bg} ${act.color} ${act.border}`}>
                        <act.icon size={15} />
                      </div>
                      <span className={`font-semibold ${act.color}`}>{act.type}</span>
                    </div>
                  </td>
                  <td className="py-3 px-6 text-gray-700 flex items-center gap-2 mt-1">
                    <img src={`https://ui-avatars.com/api/?name=${act.user.replace(' ','+')}&background=random`} className="w-6 h-6 rounded-full" />
                    <span className="font-medium">{act.user}</span>
                  </td>
                  <td className="py-3 px-6 text-gray-500">{act.date}</td>
                  <td className="py-3 px-6 text-gray-500">{act.time}</td>
                  <td className="py-3 px-6 text-gray-600 truncate max-w-md" title={act.details}>{act.details}</td>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-[11px] text-gray-500">
          <span>Affichage de 1 à 8 sur 1 248 éléments</span>
          <div className="flex items-center gap-2">
            <select className="border border-gray-200 rounded px-2 py-1.5 bg-white font-medium text-[11px]"><option>10 par page</option></select>
