...
export function BoitesArchivesPage() {
  const items = [
    { code: 'BX-2024-01', nom: 'Factures Q1 2024', location: 'Bâtiment A / Armoire 1 / Rayon 2', docs: 124 },
    { code: 'BX-2024-02', nom: 'Contrats 2024', location: 'Bâtiment A / Armoire 2 / Rayon 1', docs: 45 },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 font-poppins pb-10 text-gray-800 h-[calc(100vh-140px)]">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h2 className="text-3xl font-bold font-oswald text-gray-900">Boîtes d'archives</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col flex-1">
        ...
        <table className="w-full text-left text-[12px] whitespace-nowrap">
          <thead><tr className="bg-gray-50 border-b border-gray-100 text-gray-500 font-bold"><th className="py-3 px-5">Code</th><th className="py-3 px-5">Nom</th><th className="py-3 px-5">Emplacement</th><th className="py-3 px-5">Documents</th><th className="py-3 px-5 text-right">Actions</th></tr></thead>
          <tbody>{items.map(d=><tr key={d.code} className="border-b border-gray-50 hover:bg-gray-50/50"><td className="py-3 px-5 font-semibold text-amber-600">{d.code}</td><td className="py-3 px-5 font-bold text-gray-800">{d.nom}</td><td className="py-3 px-5">{d.location}</td><td className="py-3 px-5">{d.docs}</td><td className="py-3 px-5 text-right flex justify-end gap-2"><button className="p-1 hover:text-orange-500"><Edit2 size={14}/></button><button className="p-1 hover:text-red-500"><Trash2 size={14}/></button></td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );
