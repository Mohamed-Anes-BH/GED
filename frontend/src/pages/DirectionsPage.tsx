...
import { Building, Search, Plus, Edit2, Trash2, X, Loader2 } from 'lucide-react';
import { useOrganizationCrud } from '../hooks/useOrganization';
import { directionsService } from '../services/organization';

export function DirectionsPage() {
  const { items: directions, loading, create, update, remove, searchQuery, setSearchQuery } = useOrganizationCrud(directionsService);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<any>(null);
  
  const [formData, setFormData] = useState({ name: '', code: '', status: 'actif' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleOpenModal = (item?: any) => {
    if (item) {
      setEditingItem(item);
      setFormData({ name: item.name, code: item.code || '', status: item.status || 'actif' });
    } else {
      setEditingItem(null);
      setFormData({ name: '', code: '', status: 'actif' });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="flex flex-col gap-6 font-poppins pb-10 text-gray-800 dark:text-[var(--dash-text)] h-[calc(100vh-140px)]">
      <div className="flex items-start gap-4">
        <div className="w-14 h-14 bg-blue-50 border border-blue-100 rounded-2xl flex items-center justify-center shadow-sm shrink-0">
          <Building size={26} className="text-blue-500" />
        </div>
        <div>
          <h2 className="text-3xl font-bold font-oswald text-gray-900 dark:text-[#FFFFFF]">Directions</h2>
          <div className="flex items-center gap-1 text-[11px] font-medium text-gray-400 mt-0.5">
            <span>Accueil</span> <span>›</span> <span className="text-orange-500 font-semibold">Directions</span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            ...
            <tbody>
              {loading ? <tr><td colSpan={5} className="py-4 text-center text-gray-500">Chargement...</td></tr> : 
                directions.length === 0 ? <tr><td colSpan={5} className="py-8 text-center text-gray-500">Aucune direction trouvée.</td></tr> :
                directions.map(d=><tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-[var(--dash-border)] dark:bg-[var(--dash-bg)]/50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-[var(--dash-card-bg)] rounded-2xl shadow-lg w-full max-w-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-[var(--dash-border)]">
               <h3 className="font-bold text-gray-900 dark:text-[#FFFFFF]">{editingItem ? 'Modifier la direction' : 'Nouvelle direction'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
               ...
               <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-gray-600 dark:text-[var(--dash-text-muted)]">Nom de la direction</label>
                  <input required value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} type="text" className="px-3 py-2 border border-gray-200 dark:border-[var(--dash-border)] rounded-lg text-sm bg-transparent outline-none focus:border-orange-500" placeholder="Ex: Direction Générale" />
               </div>
               <div className="flex flex-col gap-1.5">
