...
## 📝 Changelog v2.1 (corrections vs v2.0)

1. **`courriers_diffusion` + `courriers_diffusiondestinataire` ajoutées** — présentes dans `backend_structure.md` (modèle `Diffusion`) mais absentes de ce fichier en v2.0 (saut direct de la table 22 à 23). Reconstruites ici avec une vraie table de suivi de lecture par destinataire (un simple M2M ne permet pas de savoir QUI a lu, seulement QUI est destinataire).
2. **`confidentialite` ajouté à `documents_document`** — champ manquant en v2.0 alors que l'UI validée montre un badge "Confidentiel" sur chaque document.
3. **Corbeille rendue cohérente** — `is_deleted` / `deleted_at` ajoutés à `dossiers_dossier`, `courriers_courrierentrant`, `courriers_courriersortant`. En v2.0 seul `documents_document` les avait, alors que la page Corbeille de l'UI affiche Documents + Dossiers + Courriers ensemble.
4. Numérotation des tables 26+ décalée de +2 suite à l'insertion de la diffusion.
...
Nombre de tables : 32 tables métier (+ tables Django internes)
Encoding       : UTF-8
...
| `confidentialite` | `VARCHAR(20)` | `DEFAULT 'interne'` ★ | `public`, `interne`, `confidentiel`, `secret` |
...
**Index :** `status`, `category_id`, `direction_id`, `departement_id`, `dossier_id`, `created_by_id`, `is_deleted`, `is_archived`, `confidentialite`, `created_at`
...
**Index :** `parent_id`, `direction_id`, `responsable_id`, `is_deleted`
...
**Index :** `numero`, `statut`, `priorite`, `date_reception`, `direction_id`, `assigned_to_id`, `is_deleted`
...
**Index :** `numero`, `statut`, `priorite`, `date_envoi`, `auteur_id`, `is_deleted`
...
## 26. `courriers_diffusion` — Diffusion de Courriers ★ NOUVEAU

> Corrige l'absence de cette table en v2.0 (elle était pourtant listée comme modèle dans `backend_structure.md`). Diffuse un courrier (entrant ou sortant) vers plusieurs utilisateurs.

| Colonne | Type | Contraintes | Description |
|---------|------|-------------|-------------|
| `id` | `BIGSERIAL` | `PK` | Identifiant |
| `courrier_type` | `VARCHAR(10)` | `NOT NULL` | `entrant` ou `sortant` |
| `courrier_id` | `BIGINT` | `NOT NULL` | ID du courrier (référence polymorphique simple, pas de FK stricte SQL) |
| `note` | `TEXT` | `NULL` | Note accompagnant la diffusion |
| `date_diffusion` | `TIMESTAMP` | `DEFAULT NOW()` | Date d'envoi |
| `date_expiration` | `TIMESTAMP` | `NULL` | Date d'expiration de la diffusion |
| `statut` | `VARCHAR(20)` | `DEFAULT 'en_cours'` | `en_cours`, `terminee` |
| `created_by_id` | `BIGINT` | `FK → users_user.id, NOT NULL` | Utilisateur ayant diffusé |

**Index :** `(courrier_type, courrier_id)`

---

## 27. `courriers_diffusiondestinataire` — Destinataires & Suivi de Lecture ★ NOUVEAU

> Table de liaison (remplace un simple M2M) pour permettre le suivi individuel de lecture visible dans l'UI ("État de lecture" par destinataire).

| Colonne | Type | Contraintes | Description |
|---------|------|-------------|-------------|
| `id` | `BIGSERIAL` | `PK` | Identifiant |
| `diffusion_id` | `BIGINT` | `FK → courriers_diffusion.id, NOT NULL` | Diffusion concernée |
| `user_id` | `BIGINT` | `FK → users_user.id, NOT NULL` | Destinataire |
| `lu` | `BOOLEAN` | `DEFAULT FALSE` | Lu par ce destinataire |
| `date_lecture` | `TIMESTAMP` | `NULL` | Date de première lecture |

**Contrainte unique :** `(diffusion_id, user_id)`

---

## 28. `courriers_courrierhistorique` — Historique des Courriers
...
## 29. `workflow_workflow` — Définitions de Workflow
...
## 30. `workflow_workflowstep` — Étapes de Workflow
...
## 31. `workflow_workflowinstance` — Instances en Cours
...
## 32. `workflow_stepexecution` — Exécution des Étapes
...
## 34. `ocr_ocrresult` — Résultats OCR
...
## 36. `notifications_notification` — Notifications
...
## 37. `audit_auditlog` — Journal d'Audit
...
## 38. `settings_app_appsettings` — Paramètres Application
...
## 39. `settings_app_backuprecord` — Historique des Sauvegardes
...
| Métrique | Valeur |
|----------|--------|
| **Nombre total de tables** | 39 (+ tables Django internes) |
| **Tables métier** | 32 (+2 vs v2.0 : diffusion + diffusiondestinataire) |
| **Clés étrangères (FK)** | ~62 |
| **Relations Many-to-Many** | 1 (`document_tags`) — `Diffusion.destinataires` converti en table de liaison explicite pour le suivi de lecture |
| **Champs JSONB** | 2 (`audit.details`, `courrier_historique.details`) |
| **Tables GenericFK** | 2 (`piecejointe`, `courrier_historique`) |
| **Index personnalisés** | ~38 |
...
### Document `confidentialite` ★ nouveau
```
...
> ✅ Ce fichier constitue la spécification complète (v2.1) de la base de données PostgreSQL pour AgrOdiv GED. Il peut servir de référence pour l'implémentation des `models.py` Django et pour la documentation du mémoire/PFE.
